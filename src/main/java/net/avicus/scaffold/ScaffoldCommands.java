...
public class ScaffoldCommands {
  ...
  public static void archive(CommandContext cmd, CommandSender sender) {
    ...
    try {
      ...
      if (delete) {
        ...
        sender.sendMessage(
            ChatColor.GOLD + "You have deleted and archived \"" + wrapper.getName() + "\".");
      } else {
    ...
    for (Entity entity : wrapper.getWorld().get().getEntities()) {
      if (entity instanceof Player) {
        ...
        player.sendMessage(ChatColor.RED + sender.getName()
            + " is unloading this world... Teleporting elsewhere!");
  ...
  public static void download(CommandContext cmd, CommandSender sender) {
    ...
    Bukkit.broadcastMessage(
        ChatColor.YELLOW + "World download by " + sender.getName() + " beginning...");

    Scaffold.instance().async(new Runnable() {
      @Override
      public void run() {
        try {
          HttpResponse<InputStream> response = Unirest.get(link).header("content-type", "*/*")
              .asBinary();
